...
export async function dateUpdate() {
  ...
  query.where = "project = 'N2'" + ' AND ' + "category = 'Compiled'";
...
export async function generateHandedOverArea(superurgent: any, contractcp: any) {
  // Query
  const queryContractp = `${cpField} = '` + contractcp + "'";
  const querySuperUrgentCp = querySuperUrgent + ' AND ' + queryContractp;
  const lotIdNotNull = `${lotIdField} IS NOT NULL`;
...
export async function timeSeriesHandedOverChartData(contractp: any, superurgent: any) {
  ...
  if (superurgent === superurgent_items[0]) {
    ...
    if (contractp === 'All') {
      ...
      lotLayer.definitionExpression = querySuperUrgent + ' AND ' + queryHandedOverHandOverDate;
    } else {
      query.where = querySuperUrgentCp + ' AND ' + queryHandedOverHandOverDate;
      lotLayer.definitionExpression = querySuperUrgentCp + ' AND ' + queryHandedOverHandOverDate;
    }
  }
