...
import { useSuperUrgentLotContext } from './SuperUrgentLotContext';
...
const LotProgressChart = () => {
  ...
  useEffect(() => {
    ...
    // });
    timeSeriesHandedOverChartData(cpValueSelected, superUrgentSelected).then((result: any) => {
      setLotProgressData(result);
    });
  }, [cpValueSelected, superUrgentSelected]);
    ...
    function makeSeries(name: any, fieldName: any, color: any) {
      ...
      series.columns.template.events.on('click', (ev) => {
        ...
        if (superUrgentSelected === superurgent_items[0]) {
          ...
          if (cpValueSelected === 'All') {
            query.where = querySuperUrgent + ' AND ' + qSelected + ' AND ' + qDate;
            layerViewFilter = querySuperUrgent + ' AND ' + qSelected + ' AND ' + qDate;
          } else {
            query.where = querySuperUrgentCp + ' AND ' + qSelected + ' AND ' + qDate;
            layerViewFilter = querySuperUrgentCp + ' AND ' + qSelected + ' AND ' + qDate;
          }
        }
